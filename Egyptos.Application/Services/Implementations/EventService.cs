...
public class EventService(ApplicationDbContext context) : IEventService
{
    ...
    public async Task<Result<CreateEventResponse>> AddAsync(CreateEventRequest request)
    {
        var isExstingName = await _context.Events.AnyAsync(x => x.Name == request.Name);
        if(isExstingName)
            return Result.Failure<CreateEventResponse>(EventErrors.DuplicatedEventName);

       if(await _context.EventTypes.AnyAsync(x => x.Id == request.EventTypeId) == false)
            return Result.Failure<CreateEventResponse>(EventErrors.EventTypeNotFount);
        ...
        return Result.Success(event1.Adapt<CreateEventResponse>());
    }
