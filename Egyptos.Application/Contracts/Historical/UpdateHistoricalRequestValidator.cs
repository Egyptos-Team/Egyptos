using Egyptos.Application.Contracts.EventDateContracts;
using FluentValidation;
using Microsoft.AspNetCore.Http;

namespace Egyptos.Application.Contracts.Historical;

public class UpdateHistoricalRequestValidator : AbstractValidator<UpdateHistoricalRequest>
{
    public UpdateHistoricalRequestValidator()
    {
        RuleFor(s => s.Title)
             .NotEmpty();

        RuleFor(s => s.Description)
              .NotEmpty();

        RuleFor(s => s.Date)
              .NotEmpty();

        RuleFor(s => s.Image)
            .NotNull()
            .Must(ValidateFile)
            .WithMessage("Invalid file. Allowed file types are .jpg, .jpeg, .png, and max size is 5MB.");


        RuleFor(s => s.Images)
            .NotNull()
            .Must(ValidateFiles)
            .WithMessage("Invalid file. Allowed file types are .jpg, .jpeg, .png, and max size is 5MB.");
    }
    private static bool ValidateFiles(List<IFormFile> files)
    {
        foreach (var file in files)
        {
            if (file == null || file.Length == 0)
                return false;

            var allowedExtensions = new[] { ".jpg", ".jpeg", ".png" };
            var extension = Path.GetExtension(file.FileName).ToLower();

            if (!allowedExtensions.Contains(extension))
                return false;


            const long maxFileSize = 5 * 1024 * 1024; // 5MB
            if (file.Length > maxFileSize)
                return false;
        }
        return true;
    }
    private static bool ValidateFile(IFormFile file)
    {
        if (file == null || file.Length == 0)
            return false;

        var allowedExtensions = new[] { ".jpg", ".jpeg", ".png" };
        var extension = Path.GetExtension(file.FileName).ToLower();

        if (!allowedExtensions.Contains(extension))
            return false;


        const long maxFileSize = 5 * 1024 * 1024; // 5MB
        return file.Length <= maxFileSize;
    }
}
